...
import { Users, Info, ShieldCheck, Download, Search, Trash2, Save, RefreshCw, X, CheckSquare, Square, UserMinus } from 'lucide-react';
...
export default function CustomerRegisterPage() {
  ...
  const handleAddCustomer = (e: React.FormEvent) => {
    ...
    const newCust: Customer = {
      ...
      cardNo: newCustomerCard,
      category: newCustomerCategory,
      members: newCustomerMembers,
  ...
  const filteredCustomers = customers.filter(c => {
    ...
    if (filterCategory !== 'All Categories') {
      const catKey = filterCategory.split(' ')[0]; // E.g., BPL, APL, AAY
      matchesCategory = c.category.includes(catKey);
    }

    // Status filter (Verified vs Pending Mobile Link)
    let matchesStatus = true;
    if (filterStatus === 'Verified Only') {
      matchesStatus = c.mobile !== '—';
      ...
      <section className={styles.tableCard}>
        <div className={styles.tableHeader}>
          ...
          <div className={styles.tableSyncInfo}>
            ...
            <span className={styles.syncIconBtn} onClick={() => alert('Refreshing Registry...')} title="Sync Now">
              <RefreshCw size={14} />
            </span>
          </div>
        </div>
